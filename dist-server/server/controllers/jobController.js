...
import { jobDb } from '../services/supabaseService.js';
import { processGenerationInBackground } from '../services/generationService.js';
import { applyPreset, getPreset } from '../services/presetService.js';
import { getImageFromRequest } from '../utils/requestUtils.js';
export const createGenerationJob = async (req, res, next) => {
    try {
        const jobId = uuidv4();
        const { prompt, brandKitPrompt, brandKit, shotRecipePrompt, shotRecipe, strictness, perspective, setting, plating, instructions, preset, webhookUrl, quality = 'standard', dishName, isInpaintingMode, maskImage } = req.body;
        ...
        if (brandKit) {
            const kit = getPreset('brandKits', brandKit);
            if (kit)
                finalBrandKitPrompt = finalBrandKitPrompt || kit.promptFragment;
        }
        const config = {
...
export const createBatchVariants = async (req, res, next) => {
    try {
