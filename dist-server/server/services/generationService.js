...
export const generateBackendImage = async (promptParts) => {
...
export const generateWithRetry = async (base64ImageData, mimeType, config, maxRetries = 3, promptParts) => {
    let lastError = null;
...
export const callWebhook = async (webhookUrl, data) => {
    try {
...
export const processGenerationInBackground = async (jobId, image, config, webhookUrl, isInpaintingMode = false, maskImage) => {
    try {
